...
const TenantManagement: React.FC = () => {
  ...
  const { tenants, loading, totalPages, totalElements, error, refetch } = useTenants(search, page, sort);
  
  const [showForm, setShowForm] = useState(false);
  const [editingTenant, setEditingTenant] = useState<Tenant | null>(null);
  const [tenantForm, setTenantForm] = useState<TenantRequest>({ tenantId: '', tenantName: '', tier: 'Standard' });
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await tenantApi.updateTenant(editingTenant.tenantId, { 
      ...
      setModal({ 
        ...
        type: 'success', 
        onConfirm: () => { setModal({ ...modal, isOpen: false }); setEditingTenant(null); refetch(); } 
      });
    } catch (err) { 
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-1 justify-end gap-3 min-w-[300px]">
          ...
          />
          <button 
            onClick={() => { setShowForm(!showForm); setEditingTenant(null); }} 
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="flex flex-wrap items-end gap-4 rounded-lg border border-[#444] bg-[#1e1e1e] p-6 shadow-lg">
          <div className="flex-1 min-w-[150px]">
            ...
            <input required value={tenantForm.tenantId} onChange={e => setTenantForm({...tenantForm, tenantId: e.target.value})} className="w-full rounded-md border border-[#555] bg-[#2c2c2c] p-2 text-white" />
          </div>
          <div className="flex-1 min-w-[150px]">
            <label className="mb-1 block text-sm text-[#aaa]">고객사 이름</label>
            <input required value={tenantForm.tenantName} onChange={e => setTenantForm({...tenantForm, tenantName: e.target.value})} className="w-full rounded-md border border-[#555] bg-[#2c2c2c] p-2 text-white" />
          </div>
          <div className="w-32">
            <label className="mb-1 block text-sm text-[#aaa]">서비스 티어</label>
            <select value={tenantForm.tier} onChange={e => setTenantForm({...tenantForm, tier: e.target.value})} className="w-full rounded-md border border-[#555] bg-[#2c2c2c] p-2 text-white">
      ...
      {editingTenant && (
        <form onSubmit={handleUpdate} className="flex flex-wrap items-end gap-4 rounded-lg border border-[#339af0] bg-[#1e1e1e] p-6 shadow-lg">
          <div className="flex-1 min-w-[200px]">
            <label className="mb-1 block text-sm text-[#aaa]">수정 중: {editingTenant.tenantId}</label>
            <input required value={editingTenant.tenantName} onChange={e => setEditingTenant({...editingTenant, tenantName: e.target.value})} className="w-full rounded-md border border-[#555] bg-[#2c2c2c] p-2 text-white" />
          </div>
          <div className="w-32">
            <label className="mb-1 block text-sm text-[#aaa]">서비스 티어</label>
            <select value={editingTenant.tier} onChange={e => setEditingTenant({...editingTenant, tier: e.target.value})} className="w-full rounded-md border border-[#555] bg-[#2c2c2c] p-2 text-white">
              <option>Premium</option><option>Standard</option><option>Basic</option>
            </select>
          </div>
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setEditingTenant(null)} className="rounded-md border border-[#444] bg-transparent px-6 py-2 text-[#888] transition-colors hover:bg-[#252525]">취소</button>
          </div>
        </form>
      )}
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#2c2c2c] text-sm font-semibold text-[#888]">
                <th onClick={() => handleSort('tenantId')} className="cursor-pointer px-6 py-4 text-left hover:text-white">ID</th>
                <th onClick={() => handleSort('tenantName')} className="cursor-pointer px-6 py-4 text-left hover:text-white">이름</th>
            ...
            <tbody className="divide-y divide-[#333]">
              {tenants.map((t) => (
                <tr key={t.tenantId} className="transition-colors hover:bg-[#252525]">
                  <td className="px-6 py-4 text-[#888] font-mono whitespace-nowrap">{t.tenantId}</td>
                  <td className="px-6 py-4 font-bold text-white">{t.tenantName}</td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-center gap-2">
                      <button 
                        onClick={() => { setEditingTenant({...t}); setShowForm(false); }} 
                      ...
                      <button 
                        onClick={() => setModal({
                          isOpen: true,
                          title: '삭제 확인',
                          message: `정말 고객사(${t.tenantId})를 삭제하시겠습니까?`,
                          type: 'danger',
                          onConfirm: () => handleDelete(t.tenantId)
                        })} 
