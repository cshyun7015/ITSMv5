...
import ServiceCatalogPage from '../index';

interface ServiceCatalogAdminPageProps {
  user: any;
}

export const ServiceCatalogAdminPage: React.FC<ServiceCatalogAdminPageProps> = ({ user }) => {
  const [viewMode, setViewMode] = useState<'MANAGEMENT' | 'REQUEST'>('MANAGEMENT');
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setModal({ isOpen: true, title: '수정 완료', message: '정보가 반영되었습니다.', type: 'success', onConfirm: () => { setModal({ ...modal, isOpen: false }); setEditingCatalog(null); fetchCatalogs(); } });
    } catch (e: any) {
    ...
    <div style={{ padding: '1rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem', borderBottom: '1px solid #333' }}>
        <button 
          ...
          style={{ 
            padding: '1rem 2rem', 
            backgroundColor: viewMode === 'MANAGEMENT' ? '#339af0' : 'transparent', 
            color: viewMode === 'MANAGEMENT' ? '#fff' : '#888', 
            border: 'none', 
            borderBottom: viewMode === 'MANAGEMENT' ? '3px solid #1c7ed6' : '3px solid transparent',
            cursor: 'pointer',
            fontWeight: 'bold'
          }}
        >
        ...
        <button 
          ...
          style={{ 
            padding: '1rem 2rem', 
            backgroundColor: viewMode === 'REQUEST' ? '#51cf66' : 'transparent', 
            color: viewMode === 'REQUEST' ? '#fff' : '#888', 
            border: 'none', 
            borderBottom: viewMode === 'REQUEST' ? '3px solid #37b24d' : '3px solid transparent',
            cursor: 'pointer',
            fontWeight: 'bold'
          }}
        >
          ...
          {(showForm || editingCatalog) && (
            <form onSubmit={showForm ? handleCreate : handleUpdate} style={{ backgroundColor: '#1e1e1e', padding: '2rem', borderRadius: '12px', border: `1px solid ${editingCatalog ? '#339af0' : '#444'}`, marginBottom: '2rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', marginBottom: '1.5rem' }}>
                ...
                <div>
                  <label style={{ display: 'block', color: '#aaa', fontSize: '0.85rem', marginBottom: '0.5rem' }}>아이콘</label>
              ...
              <div style={{ marginBottom: '2rem', padding: '1.5rem', backgroundColor: '#181818', borderRadius: '12px', border: '1px dashed #333' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '2fr 2fr 2fr 1fr auto', gap: '0.75rem', alignItems: 'flex-end', padding: '1rem', backgroundColor: '#252525', borderRadius: '8px' }}>
                  <input placeholder="필드 명" value={newField.fieldName} onChange={e => setNewField({...newField, fieldName: e.target.value})} style={{ padding: '0.5rem', borderRadius: '6px', border: '1px solid #444', backgroundColor: '#1e1e1e', color: '#fff' }} />
                  <input placeholder="표시 라벨" value={newField.fieldLabel} onChange={e => setNewField({...newField, fieldLabel: e.target.value})} style={{ padding: '0.5rem', borderRadius: '6px', border: '1px solid #444', backgroundColor: '#1e1e1e', color: '#fff' }} />
                  <select value={newField.fieldType} onChange={e => setNewField({...newField, fieldType: e.target.value})} style={{ padding: '0.5rem', borderRadius: '6px', border: '1px solid #444', backgroundColor: '#1e1e1e', color: '#fff' }}>
                  ...
                  <label style={{ color: '#fff', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                    <input type="checkbox" checked={newField.isRequired} onChange={e => setNewField({...newField, isRequired: e.target.checked})} /> 필수
                  </label>
                  <button type="button" onClick={addField} style={{ padding: '0.6rem 1.2rem', borderRadius: '6px', border: 'none', backgroundColor: '#339af0', color: '#fff', fontWeight: 'bold' }}>추가</button>
                </div>
              </div>

              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end', borderTop: '1px solid #333', paddingTop: '1.5rem' }}>
                 <button type="submit" style={{ padding: '0.8rem 2rem', borderRadius: '8px', border: 'none', backgroundColor: editingCatalog ? '#339af0' : '#51cf66', color: '#fff', fontWeight: 'bold' }}>
                   {editingCatalog ? '수정 사항 저장' : '새 카탈로그 등록'}
                 </button>
                 <button type="button" onClick={() => { setShowForm(false); setEditingCatalog(null); }} style={{ padding: '0.8rem 2rem', borderRadius: '8px', border: '1px solid #444', backgroundColor: 'transparent', color: '#888' }}>취소</button>
              </div>
            </form>
          )}

          {loading ? (
            <div style={{ color: '#888', textAlign: 'center', padding: '3rem' }}>데이터 불러오는 중...</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
                ...
                <tbody>
                  {catalogs.map((c: ServiceCatalog) => (
                    <tr key={c.id} style={{ borderBottom: '1px solid #333' }}>
                      ...
                      <td style={{ padding: '1.2rem 1.5rem', color: '#339af0' }}>{c.category}</td>
                      <td style={{ padding: '1.2rem 1.5rem', color: c.isPublished ? '#51cf66' : '#888' }}>{c.isPublished ? '● Published' : '○ Draft'}</td>
                      <td style={{ padding: '1.2rem 1.5rem', textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'center' }}>
                          <button onClick={() => { setEditingCatalog({...c}); setShowForm(false); }} style={{ padding: '0.4rem 0.8rem', backgroundColor: '#333', border: '1px solid #444', borderRadius: '4px', color: '#fff' }}>수정</button>
                          <button onClick={(e) => confirmDelete(e, c.id, c.catalogName)} style={{ padding: '0.4rem 0.8rem', backgroundColor: '#c92a2a', border: 'none', borderRadius: '4px', color: '#fff' }}>삭제</button>
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              {totalPages > 1 && (
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '1rem', marginTop: '2rem' }}>
                  <button disabled={page === 0} onClick={() => setPage(page - 1)} style={{ padding: '0.5rem 1rem', borderRadius: '4px', border: '1px solid #444', backgroundColor: '#333', color: '#fff' }}>이전</button>
                  <span style={{ color: '#fff' }}>Page {page + 1} of {totalPages}</span>
                  <button disabled={page >= totalPages - 1} onClick={() => setPage(page + 1)} style={{ padding: '0.5rem 1rem', borderRadius: '4px', border: '1px solid #444', backgroundColor: '#333', color: '#fff' }}>다음</button>
                </div>
              )}
            </div>
          )}
